...
void protocol_process(void)
{
    if (receive_data_by_A7105(sizeof(App_protocol), (uint8 *)&receive_frame))
    {
        ...
        if ((receive_frame.f_type!= FRAME_TYPE_ACK)
            return;
        if (receive_frame.dest_ID[0] != send_fram.src_ID[0])
            return;
        if (receive_frame.dest_ID[1] != send_fram.src_ID[1])
            return;
        if (receive_frame.dest_ID[2] != send_fram.src_ID[2])
            return;
        if (receive_frame.dest_ID[3] != send_fram.src_ID[3])
            return;
        //后面还有check_sum的计算
    }
}

bool protocol_send(uint8 * data, uint8 length)
{
	uint8 i, check_sum = 0;
    ...
    buffer[length + PROTOCOL_HEAD_LENGTH - 1] = check_sum;
...
bool protocol_receive(uint8 * data, uint8 length)
{
	uint8 i;
